...
function findFilesByExt(dir: string, ext: string): string[] {
  ...
  for (const entry of readdirSync(dir)) {
    ...
    if (statSync(full).isDirectory()) {
      files.push(...findFilesByExt(full, ext));
...
const htmlFiles = findFilesByExt(DIST, '.html');
const cssFiles = findFilesByExt(DIST, '.css');
check(htmlFiles.length > 0, '没有找到任何 HTML 文件');
check(cssFiles.length > 0, '没有找到任何 CSS 文件');
...
check(readerBookHtml.includes('章节目录会标出上次读到和已存书签的位置'), '/book/hongloumeng.html 缺少 目录阅读标记提示');
check(readerBookHtml.includes('已全本收录，可直接从头读到完结'), '/book/hongloumeng.html 缺少 全本可读 提示');
...
check(readerChapterHtml.includes('目录速览也会标出上次读到和已存书签'), '/book/hongloumeng/001.html 缺少 目录速览阅读标记提示');
...
check(readerChapterHtml.includes('可直接从当前章一路读到完结'), '/book/hongloumeng/001.html 缺少 全本连读 提示');
check(readerChapterHtml.includes('展开阅读工具与目录'), '/book/hongloumeng/001.html 缺少 移动端折叠工具入口');
check(readerChapterHtml.includes('data-reader-drawer-toggle'), '/book/hongloumeng/001.html 缺少 移动端工具折叠开关');
...
check(indexHtml.includes('已全本收录，可直接从头读到完结'), '首页缺少 全本可读 提示');
...
check(readingHtml.includes('已全本收录，可直接从头读到完结'), '我的阅读页缺少 全本可读 提示');
check(readingHtml.includes('下载 EPUB'), '我的阅读页缺少 下载 EPUB 入口');
check(readingHtml.includes('阅读记录备份'), '我的阅读页缺少 阅读记录备份 模块');
check(readingHtml.includes('导出阅读记录'), '我的阅读页缺少 导出阅读记录 入口');
check(readingHtml.includes('导入备份文件'), '我的阅读页缺少 导入备份文件 入口');
check(readingHtml.includes('较新的进度会优先保留'), '我的阅读页缺少 备份合并说明');
check(readingHtml.includes('也能清空这本记录'), '我的阅读页缺少 清空单本记录 说明');
check(readingHtml.includes('也支持逐条移除'), '我的阅读页缺少 逐条移除 说明');
check(readingHtml.includes('恢复默认设置'), '我的阅读页缺少 恢复默认设置 入口');
check(readingHtml.includes('批量整理阅读记录'), '我的阅读页缺少 批量整理阅读记录 模块');
check(readingHtml.includes('清空全部足迹'), '我的阅读页缺少 清空全部足迹 入口');
check(readingHtml.includes('清空全部书签'), '我的阅读页缺少 清空全部书签 入口');
check(readingHtml.includes('清空全部阅读记录'), '我的阅读页缺少 清空全部阅读记录 入口');
check(readingHtml.includes('保留当前阅读设置'), '我的阅读页缺少 保留当前阅读设置 说明');

// 书页移动端不应继续让左侧信息栏保持 sticky，否则会遮挡正文和目录。
const bundledCss = cssFiles.map((file) => readFileSync(file, 'utf-8')).join('\n');
check(
  /@media\(max-width:720px\)\{[\s\S]*?\.book-sidebar\[data-astro-cid-[a-z0-9]+\]\{position:static;top:auto\}/.test(bundledCss),
  '书页移动端缺少 book-sidebar 解除 sticky 的样式',
);
